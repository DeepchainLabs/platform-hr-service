...
//         _comparer ??= (x, y) => x === y;
//         this.values.some(x => _comparer(x, _value));
//     }
...
//         comparer ??= (x, y) => x === y;
//         return new List<T>(...this.filter(x => second.some(y => comparer(x, y))))
//     }

//     public groupBy<U extends DictionaryKey>(keySelector: (x: T) => U): Dictionary<U, List<T>> {
//         return new Dictionary(this.reduce((prev, curr) => ({
//             ...prev,
...
//         by ??= (x, y) => x === y;
//         return new List(...this.reduce((prev, curr, _, arr) => arr.some(x => by(x, curr)) ? prev : [...prev, curr], [] as T[]));
//     }
